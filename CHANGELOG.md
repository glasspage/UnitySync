...
- Prevent guest snapshot application from republishing its own delayed Unity object-change notifications. Skip redundant guest re-capture/hash work while applying snapshot objects, and add short unambiguous profiler markers for incoming dequeue/event handlers plus scene-apply and snapshot-end subphases so long handlers are directly identifiable. Replace full copies and scans of the accumulated scene-change dictionary with a bounded ordered queue and per-object key index, keeping each flush inside its existing time budget even in very large scenes. Add profiler markers for every UnitySyncSession update phase and scene-flush phase.
