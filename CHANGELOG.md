...
- Prevent guest snapshot application from republishing its own delayed Unity object-change notifications. Skip redundant guest re-capture/hash work while applying snapshot objects, avoid hierarchy/sibling reconciliation entirely for component-only incoming updates, send snapshot full-state data one component per packet, skip snapshot components that already match locally, suppress consecutive duplicate live component states before application, batch structural serialized-property commits by path depth instead of committing once per array/managed-reference field, reuse a single property lookup for scalar application, skip full serialized snapshot replay entirely for saved clean scenes whose Assets were already synchronized, and profile Udon atomic structure/value/reference/commit/verification phases separately so long handlers are directly identifiable. Replace full copies and scans of the accumulated scene-change dictionary with a bounded ordered queue and per-object key index, keeping each flush inside its existing time budget even in very large scenes. Add profiler markers for every UnitySyncSession update phase and scene-flush phase.
- Stop forcing editor player-loop ticks for collaborator and transform interpolation. Keep synchronization-driven Scene view repaints and collaborator interpolation fully responsive whenever Unity is focused, regardless of which editor pane the pointer is over; while Unity is unfocused, suspend forced Scene view repaints and hidden collaborator Transform writes so the editor can idle.
