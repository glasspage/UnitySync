...
- Prevent guest snapshot application from republishing its own delayed Unity object-change notifications. Skip redundant guest re-capture/hash work while applying snapshot objects, avoid hierarchy/sibling reconciliation entirely for component-only incoming updates, send snapshot full-state data one component per packet, skip snapshot components that already match locally, suppress consecutive duplicate live component states before application, and add short unambiguous profiler markers down through serializer/component staging so long handlers are directly identifiable. Replace full copies and scans of the accumulated scene-change dictionary with a bounded ordered queue and per-object key index, keeping each flush inside its existing time budget even in very large scenes. Add profiler markers for every UnitySyncSession update phase and scene-flush phase.
