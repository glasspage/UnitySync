...
- Prevent guest snapshot application from republishing its own delayed Unity object-change notifications. Replace full copies and scans of the accumulated scene-change dictionary with a bounded ordered queue and per-object key index, keeping each flush inside its existing time budget even in very large scenes. Add profiler markers for every UnitySyncSession update phase and scene-flush phase.
